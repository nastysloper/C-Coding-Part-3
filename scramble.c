...
bool lookup(string s)
{
    int midpoint = 86403; // midpoint of dictionary
    int min = 0; // left bound
    int max = 172806; // right bound
    int searching = 1;
    int flag = 0; // set to 1 if word has not already been scored
    ...
    while(searching)
    {
        ...
        { 
            ...
            {
                if(dictionary.words[midpoint].found == 1)
                {
                    searching = 0;
                }
                else
                {
                    dictionary.words[midpoint].found = 1;
                    flag = 1;
                    searching = 0;
                }
            }
            else if(strcmp(s, dictionary.words[midpoint+1].letters) == 0)
            {
                midpoint++;
                if (dictionary.words[midpoint].found == 1)
                {
        ...
        else if(strcmp(s, dictionary.words[midpoint].letters) == -1) // s comes before midpoint
        {
            max = midpoint;
            midpoint = max / 2;
        }
        else if(strcmp(s, dictionary.words[midpoint].letters) == 1) // s comes after midpoint
        {
            min = midpoint;
            midpoint = (max + min) / 2;
        }
...
void scramble(void)
{
    // Create a copy of grid to avoid overwriting original values
    ...
    for (int i = 0; i < DIMENSION; i++)
    {
        ...
        for (int j = 0; j < DIMENSION; j++)
        {
            grid[newX][newY] = temp[i][j];
            newX++;
        }
        newY--;
    }
    
    //Print out scrambled grid.
    for (int i = 0; i < DIMENSION; i++)
    {
        for (int j = 0; j < DIMENSION; j++)
        {
